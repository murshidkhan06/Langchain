...
def create_vector_db(file_upload) -> Chroma:
    ...
    # )
    # Convert chunks to Document objects with valid page_content strings
    processed_chunks = [str(chunk) for chunk in chunks]  # Ensure all are strings
    documents = [Document(page_content=chunk) for chunk in processed_chunks]

    # Create the FAISS vector database
    vector_db = FAISS.from_documents(documents, embeddings)

    # To save the FAISS index for future use
...
def main() -> None:
    ...
    """
    st.subheader("FinePrintDecoder - Making the unseen, seen.", divider="gray", anchor=False)
